<head>
    ...
    <body>
        <input type="text" id="query" autocomplete="false" />
...
function submitSelection(el){
            getAndDisplayResults(el.innerHTML)
        }
            ...
            input.addEventListener("input", function (e) {
                ...
                for (let i = 0; i < autoCompWords.length; i++) {
                    ...
                    for (let j = 0; j < wordBreaks.length; j++) {
                        if (substr == autoCompWords[i].substring(wordBreaks[j] + 1, wordBreaks[j] + 1 + substr.length).toUpperCase()) {
                            createAutoCompleteList(autoCompWords[i], wordBreaks[j] + 1, wordBreaks[j] + 1 + substr.length)
                            break
                        }
                    }
                }
...
const autoCompleteList = ["item 1", "item 2", "item 3", "thing 1", "thingy"]
